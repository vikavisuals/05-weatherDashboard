...
$("#getWeather").click(function (event) {
  ...
  $.ajax({
    ...
    $.ajax({
      ...
      let prevSearch = $(`<button id="${response.city.name.replace(' ', '')}Btn" class="btn btn-list"></button><br>`).text(response.city.name);
      $("#searchCol").append(prevSearch);

      // Function that runs when city name button is clicked
      $(`#${response.city.name.replace(' ', '')}Btn`).click(function (event) {
        event.preventDefault();
